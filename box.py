...
class MY_GUI():
    ...
    def set_init_window(self):
        self.init_window_name.title("TSM数据处理工具_v1.2")  # 窗口名
        # self.init_window_name.geometry('320x160+10+10')                         #290 160为窗口大小，+10 +10 定义窗口弹出时的默认展示位置
        self.init_window_name.geometry('1068x681+10+10')
        # self.init_window_name["bg"] = "pink"                                    #窗口背景色，其他背景色见：blog.csdn.net/chl0000/article/details/7657887
        # self.init_window_name.attributes("-alpha",0.9)                          #虚化，值越小虚化程度越高
        # 标签
        self.lab  = Label(self.init_window_name, text="File_Path")
        self.lab.grid(row=0, column=0)
        self.ent = Entry(self.init_window_name, width=40)
        self.ent.grid(row=0, column=1)
        self.lab = Label(self.init_window_name, text="类型")
        self.lab.grid(row=1, column=0)
        self.lab = Label(self.init_window_name, text="功能")
        ...
        self.log_test_label = Label(self.init_window_name, text="结果")
        ...
        self.log_test_Text = Text(self.init_window_name, width=70, height=9)  # 新增加框
        self.log_test_Text.grid(row=14, column=12, columnspan=10)
        # 按钮
        self.button1 = Button(self.init_window_name, text='Open', command=self.get_file_path)
        self.button1.grid(row=0, column=2)
        self.button2 = Button(self.init_window_name, text='submit', command=self.submit)
        self.button2.grid(row=0, column=3)
        self.str_trans_choice_1_button = Radiobutton(self.init_window_name, text='整体分析', variable=self.r_value, value=1,
                                                     command=self.r_print)
        self.str_trans_choice_1_button.grid(row=1, column=1)
        self.str_trans_choice_2_button = Radiobutton(self.init_window_name, text='会长关注', variable=self.r_value, value=0,
                                                     command=self.r_print)
        self.str_trans_choice_2_button.grid(row=1, column=2 )
        self.str_trans_choice_3_button = Checkbutton(self.init_window_name, text='写入EXCEL', command=self.myEvent1)
        self.str_trans_choice_3_button.grid(row=2,column=1)
        self.str_trans_choice_4_button = Checkbutton(self.init_window_name, text='进行标识', command=self.myEvent2)
        self.str_trans_choice_4_button.grid(row=2,  column=2)
        self.str_trans_to_md5_button = Button(self.init_window_name, text="处理数据", bg="lightblue", width=10,
