...
# 12-25日,开始优化性能和检查BUG.将功能对象化,改成class类的进行封闭.
...
ChoiceSheetName = '0'
open_write_to_excel_button = '0'  # 写EXCEL表开关变量  1是开启  0是关闭
compare_button = '0'  # 比较对比开关变量  1是开启  0是关闭


class GUI(Tk):
    all_info: int = 0  # 统计EXCEL写入的数量
    compare_info: int = 0  # 统计比较结果数量
    counter: int = 0  # 当前数据
...
def to_db_value(file):  # 从程序 中拿 到数据
    ...
    print("将数据写入mysql的子程序运行的时间是：{}秒".format(delta))
    app.write_log_to_Text("将数据写入mysql的子程序运行的时间是：{}秒".format(delta))
    return content
        ...
        for row in range(start_row, ws_rows_len + 1):  # 遍历方向是列,所以选择变更 值 为行的变化.进行循环
            ...
            if cells_value == '#N/A' or cells_value == None:  # 判断单元格中的值 等于'#N/A ,无法使用,进行下一个循环
                ...
                if temp_cell_value > cells_value:
                    ...
                    temp_cell_pos = [row, col]
                    # print('进行数据比较,结果是当前单元格的值 相等.例外,数据为:%s ,数据的坐标为行%s ,列 %s ' % (temp_cell_value, temp_cell_pos[0], temp_cell_pos[1]))
                else:
